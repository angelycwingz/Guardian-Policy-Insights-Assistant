...
It enables users to **upload, analyze, and query policy documents**, while also offering a **web search powered by Perplexity.ai** for external policy insights.  
...
- Query policies across the web using **Perplexity** for search.
...
- Two distinct modes:
  ...
  - **Web-based Q&A** – ask questions against results.
...
- **LLM Inference**: [Perplexity](https://perplexity.ai/)  
- **Web Search**: [Perplexity](https://perplexity.ai/)  
...
│ ├── inference.py # Perplexity inference wrapper
│ ├── db.py # Document storage + retrieval
│ ├── web_search.py # Perplexity search + summarization logic
...
PERPLEXITY_API_KEY – for inference and web search
